...
internal static class Pass80UnstripMethods
{
    internal static void DoPass(RewriteGlobalContext context)
    {
        ...
        foreach (var unityAssembly in context.UnityAssemblies.Assemblies)
        {
            ...
            foreach (var unityType in unityAssembly.MainModule.Types)
            {
                ...
                foreach (var unityMethod in unityType.Methods)
                {
                    ...
                    if (returnType == null)
                    {
                        Logger.Instance.LogTrace($"Method {unityMethod.ToString()} has unsupported return type {unityMethod.ReturnType.ToString()}");
                        methodsIgnored++;
                        continue;
                    }
                    ...
                    foreach (var unityMethodParameter in unityMethod.Parameters)
                    {
                        ...
                        if (convertedType == null)
                        {
                            ...
                            Logger.Instance.LogTrace($"Method {unityMethod.ToString()} has unsupported parameter type {unityMethodParameter.ToString()}");
                            break;
                        }
        ...
        Logger.Instance.LogInformation($"Restored {methodsUnstripped} methods");
        Logger.Instance.LogInformation($"Failed to restore {methodsIgnored} methods");
    }
