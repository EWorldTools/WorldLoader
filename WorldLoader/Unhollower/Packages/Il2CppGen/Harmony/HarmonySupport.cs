...
internal unsafe class Il2CppDetourMethodPatcher : MethodPatcher
{
    ...
    private void Init()
    {
        try
        {
            var methodField = Il2CppGenUtils.GetIl2CppMethodInfoPointerFieldForGeneratedMethod(Original);

            if (methodField == null)
            {
                var fieldInfoField =
                    Il2CppGenUtils.GetIl2CppFieldInfoPointerFieldForGeneratedFieldAccessor(Original);
        ...
        catch (Exception e)
        {
            Logger.Instance.LogWarning(
                $"Failed to init IL2CPP patch backend for {Original.FullDescription()}, using normal patch handlers: {e.Message}");
        }
    }
