...
public static class XrefScanner
{
    ...
    {
        var fieldValue = Il2CppGenUtils.GetIl2CppMethodInfoPointerFieldForGeneratedMethod(methodBase)
            ?.GetValue(null);
        ...
        {
            Logger.Instance.LogWarning($"Failed to get custom attribute for {methodBase.DeclaringType!.FullName}.{methodBase.Name}: {e.Message}. Falling back to scanning");
        }
