...
namespace WorldLoader.HookUtils
{
	public static class Logs
	{
		...
		public static string Debug(string message = null, ConsoleColor Color = ConsoleColor.White, string Name = "WorldLoader", string flag = "")
		{
			...
			return AddToLog($"{DateTime.Now.ToString("HH:mm:ss.fff")}[{Name}] [Debug] " + message, true);
		}
		...
		{
			//try {
			//	if (WorldLoader.Menu.flatLabel1 != null && WorldLoader.Menu.flatLabel2 != null)
			//		if (!IsDebug)
			...
			if (!IsWriting) File.WriteAllText("WorldLoader\\Log.txt", string.Empty);
			IsWriting = true;
			if (string.IsNullOrWhiteSpace(Data))
				File.AppendAllText("WorldLoader\\Log.txt", "\n");
			File.AppendAllText("WorldLoader\\Log.txt", Data + (NewLine ? "\n" : ""));
			return Data;
		}
	}
}