...
namespace Il2CppGen.Runtime
{
    public unsafe static class ClassInjector
    {
        ...
        static void CreateFakeAssembly()
        {
            ...
            FakeAssembly.Name.Name = Marshal.StringToHGlobalAnsi("InjectedMonoTypes");

            FakeImage.Assembly = FakeAssembly.AssemblyPointer;
            FakeImage.Dynamic = 1;
            FakeImage.Name = FakeAssembly.Name.Name;
        ...
        public static void RegisterTypeInIl2CppImpl(Type type, bool logSuccess, params INativeClassStruct[] interfaces)
        {
            ...
            if (logSuccess) Logger.Instance.LogInformation($"Registered mono type {type} in il2cpp domain");
        }
        ...
        private static bool IsMethodEligible(MethodInfo method)
        {
            ...
            {
                Logger.Instance.LogWarning($"Method {method} on type {method.DeclaringType} has unsupported return type {method.ReturnType}");
                return false;
            }
            ...
            foreach (var parameter in method.GetParameters())
            {
                ...
                {
                    Logger.Instance.LogWarning($"Method {method} on type {method.DeclaringType} has unsupported parameter {parameter} of type {parameterType}");
                    return false;
                }
            }
        ...
        private static Delegate CreateTrampoline(MethodInfo monoMethod)
        {
            ...
            body.Emit(OpCodes.Call, typeof(Logger.Instance).GetMethod(nameof(Logger.Instance.LogError))!);
        ...
        private static void HookGenericMethodGetMethod()
        {
            ...
            Logger.Instance.LogTrace($"il2cpp_object_get_virtual_method entry address: {getVirtualMethodEntryPoint}");

            var getVirtualMethodMethod = XrefScannerLowLevel.JumpTargets(getVirtualMethodEntryPoint).Single();
            Logger.Instance.LogTrace($"Xref scan target 1: {getVirtualMethodMethod}");

            var targetMethod = XrefScannerLowLevel.JumpTargets(getVirtualMethodMethod).Last();
            Logger.Instance.LogTrace($"Xref scan target 2: {targetMethod}");
            ...
            ourOriginalGenericGetMethod = NativeUtils.Detour(targetMethod, new GenericGetMethodDelegate(GenericGetMethodPatch));
            Logger.Instance.LogTrace("il2cpp_class_from_il2cpp_type patched");
        }
        ...
        private static Il2CppMethodInfo* GenericGetMethodPatch(Il2CppGenericMethod* gmethod, bool copyMethodPtr)
        {
            if (InflatedMethodFromContextDictionary.TryGetValue((IntPtr)gmethod->methodDefinition, out var methods))
            {
                ...
                Logger.Instance.LogTrace("Inflated method: " + inflatedMethod.Name);
        ...
        private static void HookClassFromType()
        {
            ...
            Logger.Instance.LogTrace($"il2cpp_class_from_il2cpp_type entry address: {classFromTypeEntryPoint}");

            var targetMethod = XrefScannerLowLevel.JumpTargets(classFromTypeEntryPoint).Single();
            Logger.Instance.LogTrace($"Xref scan target: {targetMethod}");

            if (targetMethod == IntPtr.Zero)
                return;

            ourOriginalTypeToClassMethod = NativeUtils.Detour(targetMethod, new TypeToClassDelegate(ClassFromTypePatch));
            Logger.Instance.LogTrace("il2cpp_class_from_il2cpp_type patched");
        }
        ...
        private static void HookClassFromName()
        {
            ...
            Logger.Instance.LogTrace($"il2cpp_class_from_name entry address: {classFromNameEntryPoint}");

            if (classFromNameEntryPoint == IntPtr.Zero) return;

            originalClassFromNameMethod = NativeUtils.Detour(classFromNameEntryPoint, hookedClassFromName);
            Logger.Instance.LogTrace("il2cpp_class_from_name patched");
        }
    ...
    {
        /// <summary>
        /// SimplePatch the native function at address specified in `from`, replacing it with `to`, and return a delegate to call the original native function
        /// </summary>
