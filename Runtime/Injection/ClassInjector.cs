...
public static unsafe partial class ClassInjector
{
    ...
    public static void RegisterTypeInIl2Cpp(Type type, RegisterTypeOptions options)
    {
        ...
        for (var i = 0; i < baseClassPointer.VtableCount; i++)
        {
            ...
            var monoMethodImplementation = type.GetMethod(methodName, parameters);
        ...
        if (options.LogSuccess)
            Logger.Instance.LogInformation($"Registered mono type {type} in il2cpp domain");
    }
    ...
    private static bool IsMethodEligible(MethodInfo method)
    {
        ...
        if (!IsTypeSupported(method.ReturnType))
        {
            Logger.Instance.LogWarning(
                $"Method {method.ToString()} on type {method.DeclaringType} has unsupported return type {method.ReturnType}");
            return false;
        }
        ...
        foreach (var parameter in method.GetParameters())
        {
            ...
            {
                Logger.Instance.LogWarning(
                    $"Method {method.ToString()} on type {method.DeclaringType} has unsupported parameter {parameter} of type {parameterType}");
                return false;
            }
        }
