...
public static class UnityVersionHandler
{
    ...
    internal static void RecalculateHandlers()
    {
        ...
        foreach (var type in InterfacesOfInterest)
            ...
            {
                if (valueTuple.Version > GetReady.Instance.UnityVersion) continue;

                Handlers[type] = valueTuple.Handler;
                break;
            }

        HasGetMethodFromReflection = GetReady.Instance.UnityVersion > new Version(2018, 1, 0);
        IsMetadataV29OrHigher = GetReady.Instance.UnityVersion >= new Version(2021, 2, 0);
    ...
    private static T GetHandler<T>()
    {
        ...
        Logger.Instance.LogError($"No direct for {typeof(T).FullName} found for Unity {GetReady.Instance.UnityVersion}; this likely indicates a severe error somewhere");
